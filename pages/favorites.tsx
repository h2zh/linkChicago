...
const Favorites = () => {
  ...
  // const [cachedMutatedData, setCachedMutatedData] = useState(null);
  // useEffect(() => {
  //   if (data) {
  //     setCachedMutatedData(data);
  //   }
  // }, []);
  ...
  return (
    <div className="mx-auto max-w-5xl px-10">
      <h1 className="text-2xl font-medium my-2">My Favorites</h1>
      <p className="text-sm font-md mb-3">
        <i>Refresh to view the links you just added</i>
      </p>
      {loading ? (
        ...
        <div className="columns-2 sm:columns-3 lg:columns-4 gap-5 space-y-5">
          ...
          {data?.favorites.length === 0 ? (
            <p className="text-md font-medium">
              Go and add some links to here!
            </p>
          ) : (
            data?.favorites.map((link) => (
              <div key={link.id} className="break-inside-avoid-column">
                <AwesomeLink
                  title={link.title}
